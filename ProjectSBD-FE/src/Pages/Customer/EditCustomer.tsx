...
export default function EditCustomer({ data, setEdit, refresh }: any) {
    ...
    const editData = async () => {
        setLoading(true)
        try {
            const res = await axios.put(`${Api_Url}/customer/edit`, {
                id: data.id,
                nama: name,
                telp,
                alamat
            ...
            console.log(res)
            toast.success(`Succes To Edit Customer`, { duration: 3000 })
            refresh()
            setLoading(false)
            setEdit({ value: false, data: {} })
            return;
        } catch (error: any) {
            console.log("Failed to Edit Customer :", error)
            getError(error, "Failed to Edit Customer")
            setLoading(false)
            return error;
        }
    }
            ...
            <div className="bg-white p-[3rem] shadow-default-black rounded-[1rem] flex flex-col gap-[2rem] relative w-[400px]">
                ...
                <form className="flex flex-col gap-[1rem]">
                    <div id="nama" className="text-[.9rem] flex flex-col gap-[.2rem]">
                        <p>Nama Customer</p>
                        <input type="text" placeholder="Input Nama Customer.." className="outline-none border border-black rounded-[.1rem] px-[1rem] py-[.5rem] w-full" value={name} onChange={(e) => setName(e.target.value)} />
                    </div>
