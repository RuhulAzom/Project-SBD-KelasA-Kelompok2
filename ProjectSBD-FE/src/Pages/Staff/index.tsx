...
export default function Staff() {
    ...
    const getStaff = async (page: number) => {
        try {
            const res = await axios.get(`${Api_Url}/staff?page=${page}`)
            console.log(res)
    ...
    const searchByName = async (page: number) => {
        try {
            const res = await axios.get(`${Api_Url}/staff/search?nama=${searchValue}&page=${page}`)
    ...
    const searchByTelp = async (page: number) => {
        try {
            const res = await axios.get(`${Api_Url}/staff/search?telp=${searchValue}&page=${page}`)
            setTotalPages(res.data.totalPages)
            setData([...res.data.data])
            return;
        } catch (error) {
    ...
    const searchByAlamat = async (page: number) => {
        try {
            const res = await axios.get(`${Api_Url}/staff/search?alamat=${searchValue}&page=${page}`)
            setTotalPages(res.data.totalPages)
            setData([...res.data.data])
            return;
        } catch (error) {
    ...
    const deleteStaff = async (id: string) => {
        setLoading(true)
        try {
            const res = await axios.delete(`${Api_Url}/staff/delete?id=${id}`)
            setLoading(false)
            console.log(res)
            alert("Succes To Delete Staff")
    ...
    const handleDelete = async () => {
        await deleteStaff(modalDelete.id)
        await getStaff(page)
    }
    ...
    return (
        <div className="py-[2rem] px-[1rem] md:px-[4rem] flex flex-col gap-[2rem]">
            ...
            {edit.value &&
                <EditStaff data={edit.data} setEdit={setEdit} refresh={refresh} />
            }
